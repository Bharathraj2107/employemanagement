from tkinter import *#we are importing everything from tkinter class
from tkinter import ttk
from tkinter import messagebox
from db import Database

db = Database("Employee.db")#we hv to pass database name as argument to the class database
root = Tk()
root.title("Employee Management System")
root.geometry("1920x1080+0+0")#width and height of computer 0+0 means start from x axis 0 and y-axis 0 we can vary the x and y aand screen sizealso
root.config(bg="#2c3e50")
root.state("zoomed")#initially there will be small gap at side afterwards it will cover when we use zoomed
...
#we keep in entry page and the title is shown down
#fg means map it with foreground color font nam,size,bold
#name block
lblName = Label(entries_frame, text="Name", font=("Calibri", 16), bg="#535c68", fg="white")#we hv to specifiy in which frame it should be in paranthesis
lblName.grid(row=1, column=0, padx=10, pady=10, sticky="w")
txtName = Entry(entries_frame, textvariable=name, font=("Calibri", 16), width=30)#we are packing means placing in 1st row ,0th column
...
txtAge.grid(row=1, column=3, padx=10, pady=10, sticky="w")#all the elemenets will stick in order on the left side
#doj block
...
def getData(event):
    selected_row = tv.focus()#when we click the row then it is focused
    ...
    gender.set(row[5])
    contact.set(row[6])
    txtAddress.delete(1.0, END)#if there is any address then that will be deleted
    txtAddress.insert(END, row[7])#here new address is inserted what is stored in row 7

def dispalyAll():
    tv.delete(*tv.get_children())#inside treeview call delete function inside that all rows will delete#by doing this all the details will be printed once  but it was initially printing the details as many times the fn was called
    #here first it will clear all and then insert
    for row in db.fetch():
        tv.insert("", END, values=row)#first argu is empty we are inserting row wise END is similar to append we add row by row
...
tv['show'] = 'headings'
tv.bind("<ButtonRelease-1>", getData)#when we click the event is captured get data is new function
tv.pack(fill=X)

dispalyAll()#when the form loads it will call
root.mainloop()#this should run infinite times until we close i)#we can reduce the width by doing this